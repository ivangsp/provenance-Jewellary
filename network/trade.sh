...
function networkDown () {
  ...
  for PEER in peer0.exporterorg.trade.com peer0.importerorg.trade.com peer0.carrierorg.trade.com peer0.regulatororg.trade.com peer0.bankorg.trade.com; do
    # Remove any old containers and images for this peer
...
function replacePrivateKey () {
  ...
  if [ "$DEV_MODE" = true ] ; then
    ...
    PRIV_KEY=$(ls *_sk)
    cd "$CURRENT_DIR"
    sed -i "s/BANK_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  fi
}
...
function generateChannelArtifacts() {
  ...
  if [ "$DEV_MODE" = false ] ; then
    ...
    set -x
    configtxgen -profile $CHANNEL_PROFILE -outputAnchorPeersUpdate \
    ./channel-artifacts/BankOrgMSPanchors.tx -channelID $CHANNEL_NAME -asOrg BankOrgMSP
    res=$?
    set +x
    if [ $res -ne 0 ]; then
