...
function replacePrivateKey () {
  ...
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/EXPORTER_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml docker-compose-cas.yaml
  cd crypto-config/peerOrganizations/importerorg.trade.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/IMPORTER_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml docker-compose-cas.yaml
  cd crypto-config/peerOrganizations/carrierorg.trade.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CARRIER_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml docker-compose-cas.yaml
  cd crypto-config/peerOrganizations/regulatororg.trade.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/REGULATOR_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml docker-compose-cas.yaml
  cd crypto-config/peerOrganizations/bankorg.trade.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/BANK_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml docker-compose-cas.yaml
...
# block - ``FiveOrgsTradeOrdererGenesis`` - and one for our channel - ``FiveOrgsTradeChannel``.
...
function generateChannelArtifacts() {
  ...
  configtxgen -profile FiveOrgsTradeOrdererGenesis -outputBlock ./channel-artifacts/genesis.block
  if [ "$?" -ne 0 ]; then
  ...
  echo "#################################################################"
  configtxgen -profile FiveOrgsTradeChannel -outputCreateChannelTx ./channel-artifacts/channel.tx -channelID $CHANNEL_NAME
  if [ "$?" -ne 0 ]; then
  ...
  echo "#################################################################"
  configtxgen -profile FiveOrgsTradeChannel -outputAnchorPeersUpdate ./channel-artifacts/ExporterOrgMSPanchors.tx -channelID $CHANNEL_NAME -asOrg ExporterOrgMSP
  if [ "$?" -ne 0 ]; then
  ...
  echo "#################################################################"
  configtxgen -profile FiveOrgsTradeChannel -outputAnchorPeersUpdate ./channel-artifacts/ImporterOrgMSPanchors.tx -channelID $CHANNEL_NAME -asOrg ImporterOrgMSP
  if [ "$?" -ne 0 ]; then
  ...
  echo "#################################################################"
  configtxgen -profile FiveOrgsTradeChannel -outputAnchorPeersUpdate ./channel-artifacts/CarrierOrgMSPanchors.tx -channelID $CHANNEL_NAME -asOrg CarrierOrgMSP
  if [ "$?" -ne 0 ]; then
  ...
  echo "#################################################################"
  configtxgen -profile FiveOrgsTradeChannel -outputAnchorPeersUpdate ./channel-artifacts/RegulatorOrgMSPanchors.tx -channelID $CHANNEL_NAME -asOrg RegulatorOrgMSP
  if [ "$?" -ne 0 ]; then
  ...
  echo "#################################################################"
  configtxgen -profile FiveOrgsTradeChannel -outputAnchorPeersUpdate ./channel-artifacts/ BankOrgMSPanchors.tx -channelID $CHANNEL_NAME -asOrg  BankOrgMSP
  if [ "$?" -ne 0 ]; then
